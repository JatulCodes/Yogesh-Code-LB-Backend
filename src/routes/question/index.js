...
router.get(
	"/id=:questionId",
	passport.authenticate(["jwt"], { session: false }),
	api4Question.getQuestionById.handler
);
router.get(
	"/search-question",
	passport.authenticate(["jwt"], { session: false }),
...
router.get(
	"/vote/id=:questionId",
	passport.authenticate(["jwt"], { session: false }),
	validate("body", api4Question.isVotedQuestion.validation),
	api4Question.isVotedQuestion.handler
);
...
router.put(
	"/vote/id=:questionId&downVote=:downVote&upVote=:upVote",
	passport.authenticate(["jwt"], { session: false }),
	validate("body", api4Question.voteQuestion.validation),
	api4Question.voteQuestion.handler
);
